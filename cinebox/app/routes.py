...
from collaborative_filtering.generate_cf_recs import generate_for_user, get_engine, load_model
import os
...
# Optional: Refresh collaborative filtering recommendations for current user
@main_bp.route("/api/recommendations/cf/refresh", methods=["POST"])
def refresh_cf_for_me():
    user_id = session.get("user_id")
    if not user_id:
        return jsonify({"success": False, "error": "Not logged in"}), 401

    try:
        model_path = os.getenv("CF_MODEL_PATH", os.path.join(os.path.dirname(__file__), "..", "model", "model.pkl"))
        top_n = int(os.getenv("CF_TOP_N", "50"))
        ttl_hours = int(os.getenv("CF_TTL_HOURS", "24"))

        model = load_model(model_path)
        # Use the same engine already configured on app
        with current_app.db_engine.begin() as conn:
            generate_for_user(conn, model, int(user_id), top_n, ttl_hours)
